...
# TODO: More arguments to behave similar to range
def _range(x):
	return (_it.count() if x == inf else range(x))
...
def lumped_network(
		...
		avail_ops     = [parallel_impedance, _add], # TODO: Use
		pre_calc_depth = 0,
	):
	# TODO: Return expression tree
	...
	for i in range(1, pre_calc_depth):
		next_pre_calc = pre_calculate_exprs(avail_vals, pre_calc)
		next_pre_calc.update(pre_calc)
		pre_calc = next_pre_calc
	pre_calc_values = sorted(pre_calc.keys())

	for i in _range(max_num_comps):
		res = lumpy_helper(avail_vals, i, target, pre_calc, pre_calc_values, pre_calc_depth, max_abs_error)
...
def lumpy_helper(avail_vals, num_comps, target, pre_calc, pre_calc_values, pre_calc_depth, max_error):
	best_error = inf
	best_val = None
	best_expr  = None

	index = bisect(pre_calc_values, target)
	if index < len(pre_calc):
		best_val = pre_calc_values[index]
		best_expr = pre_calc[best_val]
		best_error = abs(target - best_val)
	if index > 0:
		val = pre_calc_values[index - 1]
		expr = pre_calc[val]
	...
	for val in avail_vals:
		if val == target:
			return ([val], val)
		if val < target:
			series_rest = target - val
			series_expr, series_val = lumpy_helper(avail_vals, num_comps - 1, series_rest, pre_calc, pre_calc_values, pre_calc_depth, max_error)
			series_error = abs(series_rest - series_val)
			if series_error < best_error:
				best_error = series_error
				best_val = series_val + val
				best_expr = [SERIES, val, *series_expr]
				if best_error <= max_error:
					return (best_expr, best_val)
		if target < val:
			parallel_rest = (val * target) / (val - target)
			parallel_expr, parallel_val = lumpy_helper(avail_vals, num_comps - 1, parallel_rest, pre_calc, pre_calc_values, pre_calc_depth, max_error)
			parallel_error = abs(parallel_rest - parallel_val)
			if parallel_error < best_error:
				best_error = parallel_error
				best_val = (val * parallel_val) / (val + parallel_val)
				best_expr = [PARALLEL, val, *parallel_expr]
				if best_error <= max_error:
					return (best_expr, best_val)
	return (best_expr, best_val)

def pre_calculate_exprs(vals, pre_calc):
	result = {}
	for val in vals:
		for pre_calc_val in pre_calc:
			expr = pre_calc[pre_calc_val]
			series_val = val + pre_calc_val
			result[series_val] = [SERIES, val, *expr]
			parallel_val = (val * pre_calc_val) / (val + pre_calc_val)
			result[parallel_val] = [PARALLEL, val, *expr]
	return result
